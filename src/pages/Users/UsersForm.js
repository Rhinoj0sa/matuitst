...
export default function UsersForm(props) {
  ...
  const validate = (fieldValues = values) => {
    let temp = { ...errors };
    let relocationObject = { relocation: true }
    ...
    if ("age" in fieldValues) {
      temp.age = userService.validateAge(fieldValues.age)
       if (temp.age.length === 0)  relocationObject.relocation = (!(25 < parseInt(fieldValues.age) < 30))
    };
 
  setErrors({
    ...temp,
  });
  setReloc({...relocationObject.relocation})
